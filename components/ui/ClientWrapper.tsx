...
export function DataTraceCard({ title, description }: DataTraceCardProps) {
    return (
        <motion.div
            ...
            className="group relative bg-[#e0e5ec] p-8 md:p-12 rounded-[2rem] md:rounded-[2.5rem] shadow-[15px_15px_30px_#bebebe,-15px_-15px_30px_#ffffff] border-t border-l border-white/60 transition-all duration-300 cursor-pointer"
        >
        ...
        <div className="grid grid-cols-2 gap-3 sm:gap-6 p-4 md:p-10 w-full max-w-2xl">
            {cards.map((card, i) => (
                <motion.div
                    ...
                    className="group/card relative bg-[#e0e5ec] p-4 sm:p-6 rounded-2xl sm:rounded-3xl shadow-[8px_8px_16px_#bebebe,-8px_-8px_16px_#ffffff] border border-white/40 flex flex-col items-center text-center gap-2 sm:gap-3 hover:shadow-[12px_12px_24px_#bebebe,-12px_-12px_24px_#ffffff] transition-all duration-300 cursor-pointer"
                >
...
export function IntelligenceCard({ title, code, desc }: IntelligenceCardProps) {
    return (
        <motion.div
            whileHover={{ y: -10 }}
            className="p-10 bg-[#e0e5ec] rounded-[2.5rem] shadow-[15px_15px_30px_#bebebe,-15px_-15px_30px_#ffffff] border-t border-l border-white/40 group cursor-pointer"
        >
